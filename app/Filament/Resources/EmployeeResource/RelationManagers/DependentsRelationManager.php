        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('full_name')
                    ->label('Ayant Droit')
                    ->getStateUsing(fn(Dependent $record) => $record->full_name)
                    ->searchable(['first_name', 'last_name'])
                    ->weight('bold'),

                Tables\Columns\BadgeColumn::make('relationship')
                    ->label('Lien')
                    ->formatStateUsing(fn(Dependent $record) => $record->getRelationshipLabel())
                    ->colors([
                        'danger' => 'spouse',
                        'success' => 'child',
                        'info' => 'father',
                ...
                Tables\Columns\TextColumn::make('age')
                    ->label('Âge')
                    ->getStateUsing(fn(Dependent $record) => $record->age)
                    ->suffix(' ans'),
                ...
                Tables\Columns\BadgeColumn::make('validation_status')
                    ->label('Statut RH')
                    ->formatStateUsing(fn(Dependent $record) => $record->validation_status_label)
                    ->colors([
                        'warning' => 'pending',
                        'success' => 'validated',
                        'danger' => 'rejected',
