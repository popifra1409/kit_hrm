...
/**
 * ⚠️ MIGRATION DE DONNÉES — À RELIRE AVANT D'EXÉCUTER.
 *
 * Cette migration NE SUPPRIME RIEN (ni positions, ni employees.position_id).
 * Elle se contente de peupler qualification_id / trade_body_id / job_title_id
 * sur employees à partir de position_id, en créant au passage les
 * TradeBody et Qualification manquants.
 *
 * Les classifications ci-dessous sont des propositions "meilleur effort" —
 * relisez notamment la liste ADMIN_GEN et les cas signalés "// ⚠️ à vérifier".
 */
return new class extends Migration
{
    ...
    private array $newTradeBodies = [
        ...
        // Si vous préférez les répartir autrement (ou en faire des JobTitle), dites-le-moi.
    ...
    public function up(): void
    {
        ...
        foreach ($this->qualificationMap as $positionId => [$tradeBodyCode, $qualificationName]) {
            ...
            if (!$tradeBody) {
                echo "⚠️  TradeBody introuvable pour le code {$tradeBodyCode} (position #{$positionId}), ignoré.\n";
                continue;
            }
        ...
        foreach ($this->jobTitleMap as $positionId => $jobTitleName) {
            ...
            if (!$jobTitle) {
                echo "⚠️  JobTitle introuvable : {$jobTitleName} (position #{$positionId}), ignoré.\n";
                continue;
            }
    ...
    {
        // Pas de rollback automatique : on ne remet pas à null par sécurité
        // (on ne sait pas distinguer ce qui était déjà renseigné manuellement après la migration).
        echo "⚠️  Rollback non automatique pour cette migration de données. Restaurez depuis une sauvegarde si besoin.\n";
    }
};
